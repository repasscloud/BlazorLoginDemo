...
namespace Cinturon360.Api.Controllers.Tui;

[Route("api/v1/tui/")]
[ApiController]
// [ServiceFilter(typeof(RequireApiKeyFilter))] // <- uncomment to add header -based key for security (call Vicky Pollard)
public sealed class TuiOrgController : ControllerBase
{
    private readonly IAdminOrgServiceUnified _orgService;
    public TuiOrgController(IAdminOrgServiceUnified orgService)
    {
        _orgService = orgService;
    }

    // ---------- READ ----------
    [HttpGet("org/list/{parentOrgId}")] //cp4q1IdaNIKFHOGQIs5-t
    public async Task<ActionResult<IReadOnlyList<OrganizationPickerDto>>> GetOrganizations(string parentOrgId, CancellationToken ct)
    {
        var orgs = await _orgService.GetAllChildrenOrgsForPickerAsync(parentOrgId, ct);
        return orgs is null ? NotFound() : Ok(orgs);
    }

    [HttpGet("org/{orgId}")] //cp4q1IdaNIKFHOGQIs5-t
    public async Task<ActionResult<OrgAggregate?>> GetOrganization(string orgId, CancellationToken ct)
    {
