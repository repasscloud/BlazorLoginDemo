---
title: start-compose.zsh
description: Canonical entry point for starting the Compose environment
weight: 10
---
...
- Standardise how Compose is invoked
- Provide safe defaults for developers
- Centralise flags such as `--build`
...
1. Validates the local environment (shell, tooling assumptions)
2. Parses command‑line arguments
3. Optionally triggers image builds
4. Brings the Compose stack up in a controlled way
...
**Purpose**  
Fast iteration on the Blazor web frontend only.

**What it does**
- Bumps the **build number only** (`N += 1`)
- Commits the change immediately
- Builds and restarts **only the Blazor container**
- Skips:
  - Database teardown
  - Migrations
  - API restart
...
**Purpose**  
Full rebuild with a **build-number increment**.

**What it does**
- Increments build number only (`N += 1`)
- Performs a **full environment reset**
...
**Purpose**  
Version bump **without changing orchestration behaviour**.
...
**What it does**
- Resets build counter
- Performs full rebuild
- Runs migrations and full seeding
...
**What it does**
- Performs full rebuild
- Resets patch and build counters
- Commits and pushes
...
- Full clean rebuild
- Full data lifecycle
- Commits and pushes the major version
...
|------|------|--------|---------|---------------|
| `--web` | Blazor only | Partial | ❌ | Build (`N+1`) |
| `--build` | Full stack | ✅ | ✅ | Build (`N+1`) |
| `--build-only` | Full stack | ✅ | ✅ | Build (`N+1`) |
| `--patch` | Full stack | ✅ | ✅ | Patch (`Z+1`) |
...
This script intentionally combines:
...
You should update `start-compose.zsh` when:

- A new common startup flag is introduced
- Compose invocation semantics change
- Developer ergonomics need improvement

You should **not** modify it for:
