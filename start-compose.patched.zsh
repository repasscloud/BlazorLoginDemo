...
# ── 🛡️ Guard: run from this script dir; use consistent Docker config ────────
SCRIPT_DIR="${0:A:h}"
cd "$SCRIPT_DIR"

# Ensure docker uses the same credentials as your interactive user
export DOCKER_CONFIG="${DOCKER_CONFIG:-$HOME/.docker}"
echo "User: $(whoami) | HOME=$HOME | DOCKER_CONFIG=$DOCKER_CONFIG | Docker ctx=$(docker context show)"
...
# ── 🐳 2a) Pull containers ───────────────────────────────────────────────────────
echo
echo "🐳 2a) Pull ONLY external base images (skip locally-built services)…"
# Adjust service names to match your compose file (likely: db, pgadmin)
docker compose pull --ignore-pull-failures db pgadmin || true
...
echo
echo
echo "🐳 2a.1) Pull db image explicitly (to surface auth errors early)"
