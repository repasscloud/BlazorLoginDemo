...
public class FlightOption
{
    ...
    public IEnumerable<InclusionBadge> InclusionBadges
    {
        get
        {
            ...
            // Connectivity
            if (Amenities.Wifi) list.Add(new InclusionBadge { Icon = "bi bi-wifi", Label = "Wi-Fi", BadgeClass = "badge-connectivity" });
            if (Amenities.Power) list.Add(new InclusionBadge { Icon = "bi bi-lightning-charge", Label = "Power", BadgeClass = "badge-connectivity" });
            if (Amenities.Usb) list.Add(new InclusionBadge { Icon = "bi bi-usb-symbol", Label = "USB", BadgeClass = "badge-connectivity" });
            if (Amenities.Ife) list.Add(new InclusionBadge { Icon = "bi bi-tv", Label = "IFE", BadgeClass = "badge-connectivity" });

            // Seating
            if (Amenities.ExtraLegroom) list.Add(new InclusionBadge { Icon = "bi bi-arrows-expand", Label = "Extra legroom", BadgeClass = "badge-seating" });
            if (Amenities.LieFlat) list.Add(new InclusionBadge { Icon = "bi bi-moon-stars", Label = "Lie-flat", BadgeClass = "badge-seating" });

            // Catering
            if (Amenities.Meal) list.Add(new InclusionBadge { Icon = "bi bi-cup-hot", Label = "Meal", BadgeClass = "badge-catering" });
            if (Amenities.Alcohol) list.Add(new InclusionBadge { Icon = "bi bi-cup-straw", Label = "Alcohol", BadgeClass = "badge-catering" });

            // Ground/Priority
            if (Amenities.PriorityBoarding) list.Add(new InclusionBadge { Icon = "bi bi-rocket-takeoff", Label = "Priority boarding", BadgeClass = "badge-priority" });

            // Baggage
            if (Amenities.CheckedBag) list.Add(new InclusionBadge { Icon = "bi bi-suitcase2", Label = "Checked bag", BadgeClass = "badge-baggage" });
...
public static class DemoData
{
    ...
    public static List<FlightOption> Create()
    {
        ...
        var caOneStop = new FlightOption {
            ...
            PriceAUD = 1190, Cabin="First", Stops=1,
